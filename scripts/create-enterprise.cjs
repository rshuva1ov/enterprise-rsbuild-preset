...
const README_TEMPLATE = (title, withFsd) => {
  const fsdRows = withFsd
    ? `| \`pnpm fsd\` | Проверка FSD (Steiger) |
...
| \`pnpm lint\` | ESLint (включая eslint-plugin-security) |
| \`pnpm lint:fix\` | ESLint с автоисправлением |
| \`pnpm lint:style\` | Stylelint |
| \`pnpm lint:style:fix\` | Stylelint с автоисправлением |
| \`pnpm format\` | Prettier |
| \`pnpm audit\` | Проверка уязвимостей в зависимостях (High/Critical) |
${fsdRows}---
  ...
  JSON.stringify(
    {
      ...
      version: "1.0.0",
      scripts: {
        dev: "pnpm exec rsbuild dev",
        build: "pnpm exec rsbuild build",
        "create-enterprise": "pnpm exec rsbuild build",
...
async function generateCommonFiles(opts) {
  ...
  writeFile(path.join(ROOT, "README.md"), README_TEMPLATE(title, withFsd));
  ...
  writeFile(path.join(ROOT, ".husky", "pre-commit"), HUSKY_PRE_COMMIT);
  writeFile(path.join(ROOT, ".husky", "pre-push"), HUSKY_PRE_PUSH);
}
...
function updatePackageJson(withFsd, withExtraDeps = true) {
  ...
  pkg.scripts.dev = "pnpm exec rsbuild dev";
  pkg.scripts.build = "pnpm exec rsbuild build";
  pkg.scripts.audit = "pnpm audit --audit-level=high";
  ...
  if (withExtraDeps && pkg.dependencies) {
    for (const [name, version] of Object.entries(EXTRA_DEPS)) {
...
async function main() {
  ...
  await generateCommonFiles({ title, withFsd, projectName });
  if (withFsd) {
  ...
  updatePackageJson(withFsd, withExtraDeps);
